...
bookRouter.post(`${baseRoute}/insertBook`, async (request, response, next) => {
  try {
    ...
    response.status(200).json({ payload: request.body, responseFromRepo: res });
  } catch (error) {
    next(error);
  }
});

bookRouter.get(`${baseRoute}/getBooks`, async (request, response, next) => {
  try {
...
bookRouter.delete(`${baseRoute}/:bookId`, async (request, response, next) => {
  try {
